    ...
    <div className="flex max-w-2xl flex-col gap-6 animate-fade-in">
      <div>
        ...
        <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
          Podes dar acesso de duas formas: atribuindo uma <strong>role</strong> (conjunto pré-definido)
          ou marcando <strong>permissões soltas</strong>. Cada gravação de permissões soltas substitui a
          lista inteira.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-foreground">Permissões soltas</h3>
        {current.roles.length > 0 ? (
          <p className="text-xs text-muted-foreground">
            Este colaborador já tem roles atribuídas ({current.roles.map((role) => role.nome).join(", ")}).
            A API não devolve os códigos de cada role aqui, por isso a lista abaixo não vem pré-marcada —
